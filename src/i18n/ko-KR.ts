...
export const koKR = {
  ...
  homepage: {
    ...
    heroDescription: '데이터를 로컬에서 안전하게 처리하고 싶은 개발자를 위한 프라이버시 우선 도구 모음입니다. 모든 처리는 브라우저에서 이루어지며, 서버로 데이터가 전송되지 않고 추적이나 데이터 수집이 없습니다. 오픈 소스로 검증 가능하며, JSON 포맷팅, 비밀번호 생성, 해시 계산, UUID 생성 등 일반적인 개발 작업을 빠르고 안전하게 처리할 수 있습니다.',
    whyItExists: '왜 만들었나요?',
    privacyFirst: '프라이버시 우선',
    privacyFirstDescription: '모든 처리가 브라우저에서 실행됩니다. 서버로 데이터가 전송되지 않고, 추적이나 분석도 없습니다. 민감한 데이터는 당신의 컴퓨터에만 남습니다.',
    fastEfficient: '빠르고 효율적',
    fastEfficientDescription: 'Command Palette로 빠른 네비게이션, 파일 드래그 앤 드롭 지원, 대용량 데이터 처리 시 브라우저가 멈추지 않도록 Web Worker를 사용합니다.',
    installablePwa: '설치 가능한 PWA',
    installablePwaDescription: '오프라인에서도 작동하고, 독립 실행형 앱으로 설치 가능하며, 새 버전이 출시되면 자동으로 업데이트됩니다.',
    openAuditable: '오픈 소스 & 검증 가능',
    openAuditableDescription: '모든 코드가 공개되어 있습니다. 각 도구가 무엇을 하고 어떻게 데이터를 처리하는지 직접 확인할 수 있습니다.',
    hostedOn: 'GitHub Pages에 정적 사이트로 호스팅됩니다. 모든 처리는 브라우저에서 이루어집니다.',
    viewOnGithub: 'GitHub에서 보기',
    quickNavigation: '빠른 네비게이션',
    quickNavigationDescription: '{cmdK} 또는 {ctrlK}를 눌러 Command Palette를 열고 원하는 도구를 빠르게 찾으세요.',
