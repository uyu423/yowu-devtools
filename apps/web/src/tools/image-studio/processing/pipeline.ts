...
// 2. Execute only enabled steps in order: crop -> rotate -> resize
// 3. Each step receives the previous step's output canvas
// 4. Export final canvas to the target format
//
// Pipeline order rationale:
// - Crop first: remove unwanted areas before other operations
// - Rotate second: transform the cropped area
// - Resize last: scale to final dimensions after all transformations
//
// Key optimizations:
...
export async function executePipeline(
  ...
  try {
    ...
    // Stage 3: Rotate/Flip (if enabled and has transformations)
