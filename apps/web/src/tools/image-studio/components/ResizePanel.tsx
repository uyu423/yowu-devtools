...
interface ResizePanelProps {
  ...
  onQualityChange: (quality: InterpolationQuality) => void;
  t: (key: string) => string;
}
...
export const ResizePanel: React.FC<ResizePanelProps> = ({
  ...
  // Generate quality options with i18n translations
  const qualityOptions = useMemo(
    () => [
      { value: 'low', label: t('common.quality.low') },
      { value: 'medium', label: t('common.quality.medium') },
      { value: 'high', label: t('common.quality.high') },
    ],
    [t]
  );
  ...
  return (
    <div className="space-y-4" data-step="resize">
      {/* Dimensions */}
      <div>
        <OptionLabel tooltip={t('tool.imageStudio.resize.dimensionsTooltip')}>
          <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">
            {t('tool.imageStudio.resize.dimensions')}
          </label>
        </OptionLabel>
        <div className="flex items-center gap-2">
          <div className="flex-1">
            <label className="text-xs text-gray-500 dark:text-gray-400">{t('tool.imageStudio.resize.width')}</label>
            <input
              type="number"
              value={width}
          ...
          <button
            ...
            title={lockAspect ? t('tool.imageStudio.resize.unlockAspect') : t('tool.imageStudio.resize.lockAspect')}
          >
            {lockAspect ? <Lock className="w-4 h-4" /> : <Unlock className="w-4 h-4" />}
          </button>
          <div className="flex-1">
            <label className="text-xs text-gray-500 dark:text-gray-400">{t('tool.imageStudio.resize.height')}</label>
            <input
              type="number"
              value={height}
        ...
        >
          {t('tool.imageStudio.resize.useOriginal')} ({originalWidth} × {originalHeight})
        </button>
      </div>
      ...
      <div>
        <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">
          {t('tool.imageStudio.resize.presets')}
        </label>
      ...
      {/* Resize Mode */}
      <div>
        <OptionLabel tooltip={t('tool.imageStudio.resize.modeTooltip')}>
          <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">
            {t('tool.imageStudio.resize.mode')}
          </label>
        </OptionLabel>
        <Select
          value={mode}
      ...
      <div>
        <OptionLabel tooltip={t('tool.imageStudio.resize.qualityTooltip')}>
          <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">
            {t('tool.imageStudio.resize.quality')}
          </label>
        </OptionLabel>
        <div className="flex gap-2">
          {qualityOptions.map((option) => (
            <button
              key={option.value}
              type="button"
              onClick={() => onQualityChange(option.value as InterpolationQuality)}
              className={cn(
