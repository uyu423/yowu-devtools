/**
 * Pipeline Preset Modal - 파이프라인 프리셋 관리 모달 (Image Studio)
 */
...
import type { ImagePipelinePreset } from '../hooks/usePipelinePresets';
...
interface PipelinePresetModalProps {
  ...
  onSelect: (preset: ImagePipelinePreset) => void;
  onAddPreset: (
    name: string,
    description: string,
    settings: ImagePipelinePreset['settings']
  ) => void;
  onRemovePreset: (id: string) => void;
  onClearAll: () => void;
  onExport: () => void;
  onImport: (file: File) => Promise<{ success: boolean; count: number; error?: string }>;
  getCurrentSettings: () => ImagePipelinePreset['settings'];
  t: (key: string) => string;
}
...
export const PipelinePresetModal: React.FC<PipelinePresetModalProps> = ({
  ...
  const handleSelect = (preset: ImagePipelinePreset) => {
    onSelect(preset);
    onClose();
