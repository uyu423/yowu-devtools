...
import type { ApiTesterState, HistoryItem, HttpMethod } from './types';
import {
  BODY_SUPPORTED_METHODS,
...
import {
  ...
  convertToApiTesterState,
  getStoredApiTesterState,
} from '@/lib/curl/convertToApiTester';
import {
  createKeyValueItem,
  parseUrlParams,
  sanitizeUrlAndParams,
  toCurlCommand,
} from './utils';
...
const ApiTesterTool: React.FC = () => {
    ...
    shareStateFilter: ({
      ...
      if (body.kind !== 'none') {
        if (body.kind === 'urlencoded') {
          ...
          const textItems = body.items.filter(
            (i) => i.type === 'text' && i.key
          );
          if (textItems.length > 0) {
            shareState.body = {
              kind: 'multipart',
              items: textItems.map(({ key, textValue }) => ({
                key,
                textValue,
              })),
            };
            ...
            action: {
              ...
              onClick: () => {
                ...
                if (parsedParams.length > 0) {
                  updateState({
                    queryParams: [...parsedParams, createKeyValueItem()],
                  });
                }
              },
            },
  ...
  const executeWithAutoMode = useCallback(
    async (requestState: ApiTesterState, forceExtension: boolean = false) => {
      const mode = forceExtension ? 'extension' : 'direct';
      console.log(
        '[API Tester] Executing request with mode:',
        mode,
        'url:',
        requestState.url
      );
  ...
  const handleSend = useCallback(async () => {
    ...
    const { sanitizedUrl, mergedParams } = sanitizeUrlAndParams(
      state.url,
      state.queryParams
    );
    ...
    const shouldUseExtension =
      isCorsAllowed(requestState.url) && extensionStatus === 'connected';

    if (shouldUseExtension) {
      console.log(
        '[API Tester] Origin in CORS allowlist, using extension automatically'
      );
    }
  ...
  const handleRetryWithExtension = useCallback(
    async (rememberChoice: boolean) => {
      console.log(
        '[API Tester] Retrying with extension, remember:',
        rememberChoice
      );
      ...
      updateState({
        ...
        headers:
          item.request.headers.length > 0
            ...
            : [createKeyValueItem('Content-Type', 'application/json')],
        body: item.request.body,
        queryParams:
          parseUrlParams(item.request.url).length > 0
            ? parseUrlParams(item.request.url)
            : [createKeyValueItem()],
      });
      clearResponse();
    },
  ...
  const handleSendToBurstTest = useCallback(() => {
    ...
    const method = supportedMethods.includes(state.method)
      ? state.method
      : 'GET';
    ...
    if (state.body.kind === 'json') {
      ...
      bodyText = state.body.items
        ...
        .map(
          (i) => `${encodeURIComponent(i.key)}=${encodeURIComponent(i.value)}`
        )
        .join('&');
    }
    ...
    navigate(buildLocalePath(locale, '/api-burst-test'), {
      state: burstTestState,
    });
    toast.success(t('tool.apiTester.sentToBurstTest'));
  }, [state, navigate, t, locale]);

  // Send to API Diff
  const handleSendToApiDiff = useCallback(
    (targetDomain: 'A' | 'B') => {
      ...
      toast.success(
        t('tool.apiTester.sentToApiDiff').replace('{domain}', targetDomain)
      );
    },
    [state, navigate, t, locale]
  );
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (
        apiDiffDropdownRef.current &&
        !apiDiffDropdownRef.current.contains(e.target as Node)
      ) {
        setApiDiffDropdownOpen(false);
      }
    };
  ...
  const activeQueryCount = state.queryParams.filter(
    (p) => p.key && p.enabled
  ).length;
  const activeHeaderCount = state.headers.filter(
    (h) => h.key && h.enabled
  ).length;
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 mt-2">
                ...
                <div className="flex items-center gap-1.5">
                  <label className="flex items-center gap-1.5 text-sm text-gray-600 dark:text-gray-400 cursor-pointer">
                    <input
                      ...
                      onChange={(e) =>
                        updateState({ includeCookies: e.target.checked })
                      }
                      disabled={isLoading}
                    ...
                    <span className="hidden sm:inline">
                      {t('tool.apiTester.includeCookies')}
                    </span>
                  </label>
                  <Tooltip
                    content={t('tool.apiTester.includeCookiesTooltip')}
                    position="bottom"
                    nowrap={false}
                  >
            ...
            <div className="flex items-center gap-1 sm:gap-2 shrink-0">
              ...
              >
                ...
                <span className="hidden sm:inline">
                  {t('tool.apiTester.sendToBurstTest')}
                </span>
              </button>
              ...
              <div className="relative" ref={apiDiffDropdownRef}>
                ...
                >
                  ...
                  <span className="hidden sm:inline">
                    {t('tool.apiTester.sendToApiDiff')}
                  </span>
                  <ChevronDown
                    className={cn(
                      'w-3 h-3 transition-transform hidden sm:block',
                      apiDiffDropdownOpen && 'rotate-180'
                    )}
                  />
                </button>
                ...
                {apiDiffDropdownOpen && (
                  ...
                  >
                    ...
                    >
                      <span className="w-5 h-5 flex items-center justify-center rounded bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 text-xs font-semibold">
                        A
                      </span>
                    ...
                    >
                      <span className="w-5 h-5 flex items-center justify-center rounded bg-emerald-100 dark:bg-emerald-900 text-emerald-700 dark:text-emerald-300 text-xs font-semibold">
                        B
                      </span>
