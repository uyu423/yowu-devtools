...
### US-23: Hash/Checksum Generator (v1.2.0)

**시나리오**: 문자열의 해시값(SHA-256 등)을 계산하고 싶다.

- [ ] **해시 계산**

  - [ ] 입력 문자열의 해시값이 즉시 계산되는지 확인
  - [ ] 알고리즘 선택 시 즉시 재계산되는지 확인
  - [ ] 여러 알고리즘 지원 확인 (SHA-256, SHA-1, MD5, SHA-384, SHA-512)

- [ ] **HMAC 옵션**

  - [ ] HMAC 옵션 토글 시 HMAC 키 입력 필드 표시 확인
  - [ ] HMAC 키 입력 시 HMAC 해시 계산 확인
  - [ ] HMAC 알고리즘 선택 확인 (HMAC-SHA-256, HMAC-SHA-1 등)

- [ ] **결과 표시**

  - [ ] 해시값이 Hex 형식으로 표시되는지 확인
  - [ ] Base64 형식 옵션 확인 (선택적)
  - [ ] 결과가 읽기 쉽게 표시되는지 확인 (줄바꿈, 공백 등)
