...
- FR-J-06: 전체화면 모드 (v1.3.4 추가)

  - 오른쪽 패널(결과 뷰)을 전체 너비로 확장하는 토글 버튼 제공
  - Copy 버튼 옆에 Fullscreen 토글 버튼 배치
  - ESC 키 또는 버튼 재클릭으로 전체화면 해제
  - 대형 JSON 탐색 시 유용

- FR-J-07: 검색 UI 최적화 (v1.3.4 추가)

  - 검색 입력 필드는 Pretty 모드에서만 표시 (하이라이트가 동작하는 뷰)
  - Tree/Minify 모드에서는 검색 입력 숨김 (깔끔한 UI)
...
### 7.14 String Length Calculator (`toolId: string-length`) (v1.3.5 추가)

#### 목적

- 텍스트의 문자, 단어, 줄, 바이트 수를 빠르게 계산하고 표시
- 파일 업로드를 통해 대용량 텍스트도 처리 가능

#### 상태

```ts
type StringLengthToolState = {
  input: string; // 텍스트 입력
};
```
...
- FR-SL-01: 텍스트 입력 필드 제공
  - 기본값: "Hello Yowu's DevTools"
  - 파일 드래그 앤 드롭 및 파일 선택 다이얼로그를 통한 텍스트 로드 지원
- FR-SL-02: 실시간으로 다음 통계 정보 계산 및 표시 (Debounce 적용)
  - **문자 수 (Characters)**: `String.length` 기준
  - **문자 수 (공백 제외)**: 모든 공백 문자(`\s`)를 제외한 문자 수
  - **단어 수 (Words)**: 공백으로 구분된 단어 수 (빈 문자열 제외)
  - **줄 수 (Lines)**: 줄바꿈 문자(`\n`) 기준으로 줄 수
  - **바이트 수 (UTF-8)**: `TextEncoder().encode().length` 기준
- FR-SL-03: 각 통계 항목별 복사 버튼 제공
- FR-SL-04: 유니코드 문자 (한글, 이모지 등)에 대한 정확한 계산 지원
- FR-SL-05: 파일 로드 시 로딩 인디케이터 표시
- FR-SL-06: 상태 저장 및 공유
  - `localStorage`에 입력값 자동 저장 및 복원
  - URL 공유 링크에 입력값 포함 및 복원

- AC
  - 입력 변경 시 300ms 이내로 모든 통계가 업데이트됨
  - 한글 "안녕하세요" 입력 시 문자 수 5, 바이트 수 15로 정확히 계산됨
  - 파일 업로드 시 내용이 정상적으로 로드되고 통계가 표시됨
