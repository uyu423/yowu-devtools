...
- **Domain A/B 입력 (상단)**
  ...
  - **Domain Preset 기능** (모달):
    ...
    - localStorage에 저장 (`yowu-devtools:api-diff:domain-presets`)
...
- `yowu-devtools:api-diff:state` (도구 상태)
- `yowu-devtools:api-diff:history` (히스토리)
- `yowu-devtools:api-diff:ui:diff-table-height` (Diff 테이블 높이 설정)
- `yowu-devtools:api-diff:domain-presets` (도메인 프리셋)
...
### Phase 2: 요청 실행 및 응답 처리 ✅ 완료

1. ✅ useApiDiffExecutor 훅 생성 (Extension 지원)
2. ✅ Extension 모드 지원 (useExtension 훅 재사용)
3. ✅ 응답 파싱 및 메타 정보 수집
...
### Phase 4: 히스토리 및 공유 ✅ 완료

1. ✅ 히스토리 저장/로드 기능 (useHistory 훅)
2. ✅ URL 공유 기능 (encodeShare, decodeShare)
3. ✅ 히스토리 UI 구현 (HistoryDropdown)

### Phase 5: 추가 기능 및 최적화 ✅ 완료
...
- **v1.4.2 (2025-12-17) UI 업데이트**:
  ...
  - i18n 키 추가 (preset 관련)
- **v1.4.2 (2025-12-17) 기능 구현 완료**:
  - useApiDiffExecutor 훅 생성 (Extension 모드 지원)
  - JSON Viewer 통합 (react-json-tree, 다크 모드 지원)
  - 비교 로직 구현 (deepEqualIgnoringKeyOrder, findDifferentFields)
  - ResultBanner 실제 응답 연동
  - 히스토리 기능 구현 (useHistory 훅, HistoryDropdown)
  - URL 공유 기능 구현 (encodeShare, decodeShare)
